...
import {removeFriend} from "../redux/auth";
...
const Profile = () => {
    ...
    const { data, isLoading, isError } = useQuery("userPosts", async () =>
        (await axios.get(`${URL.BASE_URL}/users/${userId}`)).data
    );

    const { data: data2, isLoading: isLoading2,isError:e2 } = useQuery("userFriends", async () =>
        (await axios.get(`${URL.BASE_URL}/friends/${userId}`)).data
    );
    ...
    const uploadImageMutation = useMutation(
        async () => {
            ...
            return await axios.put(`${URL.BASE_URL}/image/update/${userId}`, formData,
                {
                    headers: {
        ...
        {
            onSuccess: response => {
                queryClient.invalidateQueries("myImage");
                closeImageModal();
            },
            onError: error => {
        ...
        <div style={{
            ...
            <main>
                ...
                <div id="profile-upper">
                    ...
                    <div id="profile-d">
                        <div id="profile-pic">
                            <img src={image1.url} alt="my profile"/>
                        </div>
                        <i>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQJRG0ehYH1QqNyo6TwX1-eLmFP-qK-qQDUDg&usqp=CAU" style={{
                                ...
                                width: "40px",
                                marginTop: "-19px"
                            }} onClick={openImageModal} className="hover:cursor-pointer" alt="camera icon"/>
                        </i>
                        <Modal isOpen={isImageModalOpen} onClose={closeModal}>
                            <div className="text-red-800 m-5">
                                <input type="file" accept="image/*" onChange={handleImageChange} />
                                <button onClick={handleReplaceImage} className="" style={{
                                    marginRight: "80px"
                        ...
                        <div className="td" id="l-col">
                            ...
                            <div className="l-cnt l-mrg">
                                <div className="cnt-label">
                                    ...
                                    <i className="text-amber-900">({data2.friends.length})</i>
                                        ...
                                        <div key={post.id} className="postk">
                                            ...
                                            <div className="post-content">
                                                ...
                                                <p className="text-rose-500">{post.firstName}</p>
                                                    <button style={{
                                                        backgroundColor: "#cc0731",
                                                        borderRadius: "70px",
                                                        color: "#cee8e7"
                                                    }} onClick={() => handleRemoveFriend(post.id)}>Remove</button>
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="m-mrg" id="composer">
                                <div id="c-tabs-cvr">
                                    <div className="tb" id="c-tabs">
                                        <div className="td active"><span>Make A Status</span></div>
                                        <div className="td"><i className="material-icons" >{"                                                                            "}</i></div>
                                    </div>
                                </div>
                                <div id="c-c-main">
                                    <div className="tb">
                                        <div className="td profile-pic" id="p-c-i"><img src={image1.url} alt="Profile pic"/></div>
                                ...
                                <div className="post-list">
                                    {data.posts.map((post) => (
                                        <div key={post.id} className="postk mb-3">
                                            <img src={image1.url} alt={`Profile of ${post.username}`} className="profile-pic" />
                                            <div className="post-content">
